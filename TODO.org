* Issues & ideas
  + *[general]* Consider using perforate lein plugin to add benchmarks.
    ...
    This however need to be microbenchmarks.
  + *[tagging]* Add API DOCS to tagging module.
  + *[tagging]* Add function to tag collection of string.
    The tagger fun is created only once that way, so it shoul improve perf.
    The sample stale should follow `calc-tags-union`, where `make-norm-f` 
    creates normalizer function.
  + *[tagging]* Add some kind of gen-dict-from-file.
    Check how does with-open works (is it reasonable to take reader as param).
