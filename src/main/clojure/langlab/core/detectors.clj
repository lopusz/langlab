...
(defn detect-lang-prob-tika
  ...
  **Note.** The probability is **very conservative**. According to the apidocs for
  short texts it always gives uncertain. Even on long English texts I could not
...
(defn detect-encod-prob-icu
  "Returns a string identifier of encoding for file `fname` as
   ...
   (double from the range [0.0.1.0]). The result is a map
...
(defn detect-all-encod-prob-icu
  "Returns multiple string identifiers of encodings for file `fname` as
   ...
   is provided (double from the range [0.0.1.0]).  The result is a map
